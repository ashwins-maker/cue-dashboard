...
| A2 | **`checkout_started` emitted** | The middle of the funnel | Storefront emit on checkout navigation |
| A3 | **`cart_remove` emitted** | Bracketing's own success measure — "one of the two sizes leaves the cart" | Extend the `byond:cart-updated` handler that now emits `add_to_cart` |
...
| A5 | **`filter_applied` emitted** | Size intent — "filters the collection by her size" | Same collection-page work as A4 |
...
| C2 | **Carry `triggering_signals` through the rollup** | The per-intent "triggered by" breakdown. The column is captured per firing, then aggregated away | Keep it in the insight group-by |
...
All four signals are emitted today and route nowhere.

| # | Item | Blocks |
|---|---|---|
| D1 | **`zoom` → a rule** | Fabric never fires at all |
| D2 | **`colour_variant_change` → a rule** | Colour intent |
| D3 | **`scroll_reversal` → a rule** | Fit — "scrolls back to re-read the fit description" |
| D4 | **`multi_tab_same_product` → a rule** | Comparison — the cheap same-product-two-sizes case |
...
| E3 | **Widget bundle at 14.70kb against a 15kb budget** | 0.30kb of headroom. The next widget feature breaks it outright |
...
4. **D1 + B2 — a `zoom` rule and a fabric card type.** The content now exists; the trigger does not.
